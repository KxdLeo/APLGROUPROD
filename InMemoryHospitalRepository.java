...
public class InMemoryHospitalRepository implements HospitalRepository {
    ...
    private int nextAppointmentId = 3;
    private int nextPaymentId = 3;
    ...
    public InMemoryHospitalRepository() {
        ...
        employees.put("R4001", new Receptionist("R4001", "Kevin", "Morgan", "876-555-4001",
                "staff123", "Front Desk"));
        employees.put("A5001", new Admin("A5001", "Simone", "Clarke", "876-555-5001",
        ...
        appointments.add(new Appointment(1, "P1001", "D2001",
                LocalDateTime.now().plusDays(2).withHour(10).withMinute(0), "Scheduled", "General Check Up"));
        appointments.add(new Appointment(2, "P1002", "D2001",
                LocalDateTime.now().plusDays(4).withHour(13).withMinute(30), "Pending", "Lab Tests"));

        payments.add(new Payment(1, "P1001", new BigDecimal("3500.00"),
                LocalDate.now().minusDays(10), new BigDecimal("1200.00"),
                "Previous Visit", "Unpaid"));
    ...
    @Override
    public synchronized Patient savePatient(Patient patient) {
        patients.put(patient.getId(), patient);
        return patient;
    }

    @Override
    public synchronized Patient updatePatient(Patient patient) {
        patients.put(patient.getId(), patient);
        return patient;
    }
    ...
    @Override
    public synchronized Employee updateEmployee(Employee employee) {
        employees.put(employee.getId(), employee);
        return employee;
    }
    ...
    @Override
    public synchronized Appointment saveAppointment(Appointment appointment) {
    ...
    @Override
    public synchronized Payment savePayment(Payment payment) {
    ...
    @Override
    public synchronized void updatePayment(Payment payment) {
    ...
    @Override
    public synchronized List<ChatMessage> findChatMessages(String userId, String receiverRole) {
        List<ChatMessage> results = new ArrayList<ChatMessage>();
        for (ChatMessage message : chatMessages) {
            boolean sentByUser = userId == null || userId.length() == 0 || userId.equals(message.getSenderId());
            boolean sentToRole = receiverRole == null || receiverRole.length() == 0
                    || receiverRole.equalsIgnoreCase(message.getReceiverRole());
            if (sentByUser || sentToRole) {
                results.add(message);
            }
        }
        return results;
